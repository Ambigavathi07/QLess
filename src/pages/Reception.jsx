...
console.log("Doctors:", d.data);
console.log("Doctors API Response:", d.data);
...
doctorId: String(form.doctor_id),
        ...
        patientAge: form.patient_age ? Number(form.patient_age) : 0,
...
const byDoctor = doctors.map((d) => ({
  ...
  return (
    <AppShell
      title="Reception"
      subtitle="Tokens · payments · queue control"
      actions={
        <Select value={session} onValueChange={setSession}>
          <SelectTrigger
            className="w-[140px] h-9 border-[#E2E5E0]"
            data-testid="reception-global-session"
          >
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
    ...
    >
      <div className="grid lg:grid-cols-12 gap-6">
            ...
            >
              ...
              <Field label="Session">
                ...
                >
                  <SelectTrigger
                    className="mt-1.5 h-10 border-[#E2E5E0]"
                    data-testid="reception-session-select"
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <Field label="Patient name">
                <Input
                  ...
                  className="mt-1.5 h-10 border-[#E2E5E0]"
                  required
                  data-testid="reception-name-input"
                />
              </Field>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Phone">
                  <Input
                    ...
                    className="mt-1.5 h-10 border-[#E2E5E0]"
                    required
                    data-testid="reception-phone-input"
                  />
                </Field>
                <Field label="Age">
                  <Input
                    ...
                    className="mt-1.5 h-10 border-[#E2E5E0]"
                    data-testid="reception-age-input"
                  />
                </Field>
              </div>
              <Field label="Priority">
                ...
                >
                  <SelectTrigger
                    className="mt-1.5 h-10 border-[#E2E5E0]"
                    data-testid="reception-priority-select"
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <Button
                ...
                data-testid="reception-submit-btn"
              >
                Issue token
              </Button>
            </form>
        ...
        <div className="lg:col-span-7 space-y-6">
          {byDoctor.map((d) => {
const receptionControlled =
  d.queueControlMode === "reception";
              const waitingCount = d.tokens.filter(
              (t) => t.state === "waiting",
            ).length;
            return (
              <div
              ...
              >
                <div className="p-4 border-b border-[#E2E5E0] flex items-center justify-between gap-3 flex-wrap">
                  <div>
                    <div className="font-heading font-bold tracking-tight flex items-center gap-2">
                      ...
                      {receptionControlled && (
                        <span
                          className="text-[10px] px-1.5 py-0.5 rounded bg-[#f4d4ca] text-[#9C3A24] font-bold tracking-wide uppercase"
                          data-testid={`mode-badge-${d.id}`}
                        >
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-[11px] tracking-[0.2em] uppercase font-bold text-[#5C6661]">
                      {d.tokens.length} today · {waitingCount} waiting
                    </div>
                    {receptionControlled && (
                      <Button
                        size="sm"
                        onClick={() => callNext(d.id)}
                ...
                {d.tokens.length === 0 ? (
                  <div className="p-8 text-center text-sm text-[#5C6661]">
                    No tokens yet.
                  </div>
                ) : (
                  <div className="max-h-[340px] overflow-y-auto">
                    <table
                      ...
                      data-testid={`doctor-queue-table-${d.id}`}
                    >
                          ...
                          <tr key={t.id} className="border-t border-[#E2E5E0]">
                            <td className="p-3 font-mono font-bold">
                              ...
                              {t.priority === "emergency" && (
                                <span className="ml-1 text-[#D36A50]">!</span>
                              )}
                            </td>
                            <td className="p-3">
        ...
        <DialogContent className="max-w-sm" data-testid="token-slip">
          ...
          {slip && (
            <div className="space-y-4 text-center">
              <div className="text-[10px] tracking-[0.3em] uppercase font-bold text-[#5C6661]">
                Your token
              </div>
              <div className="font-mono text-8xl font-black tracking-tighter text-[#2A4B41] leading-none">
                {slip.token_number}
              </div>
              ...
              <div className="grid place-items-center py-2">
                <QRCodeSVG
                  value={`${window.location.origin}/q/${slip.qr_token}`}
                  size={140}
                  bgColor="#fff"
                  fgColor="#2A4B41"
                />
              ...
              <div className="text-xs text-[#5C6661]">
                Scan to track live queue position
              </div>
              <div className="flex gap-2">
                <Button
                  onClick={() => window.print()}
                  variant="outline"
                  className="flex-1 border-[#E2E5E0]"
                ...
                <Button
                  ...
                  className="flex-1 bg-[#2A4B41] hover:bg-[#1E362E] text-white"
                  data-testid="close-slip-btn"
                >
                  Done
                </Button>
              </div>
  ...
  return (
    <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Change token #{editing.token_number}</DialogTitle>
        </DialogHeader>
        <form
          onSubmit={(e) => {
            e.preventDefault();
            const send = { ...patch };
            send.patient_age =
              send.patient_age === "" ? null : Number(send.patient_age);
            onSave(send);
          }}
          className="space-y-4"
        ...
        >
          <Field label="Doctor (reassign changes token number)">
            <Select
              value={patch.doctor_id}
              onValueChange={(v) => setPatch({ ...patch, doctor_id: v })}
            >
              <SelectTrigger
                className="mt-1.5 h-10 border-[#E2E5E0]"
          ...
          <Field label="Patient name">
            <Input
              value={patch.patient_name}
              onChange={(e) =>
                setPatch({ ...patch, patient_name: e.target.value })
              }
              className="mt-1.5 h-10 border-[#E2E5E0]"
              data-testid="reassign-name"
            />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Phone">
              <Input
                value={patch.patient_phone}
                onChange={(e) =>
                  setPatch({ ...patch, patient_phone: e.target.value })
                }
                className="mt-1.5 h-10 border-[#E2E5E0]"
                data-testid="reassign-phone"
              />
            </Field>
            <Field label="Age">
              <Input
                type="number"
                value={patch.patient_age}
                onChange={(e) =>
                  setPatch({ ...patch, patient_age: e.target.value })
                }
                className="mt-1.5 h-10 border-[#E2E5E0]"
                data-testid="reassign-age"
              />
            </Field>
          </div>
          <div className="flex gap-2">
...
function PaymentDialog({ token, onClose, onSaved }) {
  ...
  useEffect(() => {
    if (token)
      setForm({ amount: "", method: "cash", type: "consultation", notes: "" });
  }, [token]);
  ...
  //         toast.success(`Token #${r.data.tokenNumber} issued`);
  ...
  return (
    <Dialog open={!!token} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>
            Record payment — token #{token.token_number}
          </DialogTitle>
        </DialogHeader>
        <form
          onSubmit={submit}
        ...
        >
          <div className="text-sm text-[#5C6661]">
            <div className="font-medium text-[#1A1D1C]">
              {token.patient_name}
            </div>
...
function StateBadge({ state, priority }) {
  ...
  return (
    <div className="inline-flex items-center gap-1">
      <span
        className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold tracking-wider uppercase ${bg} ${fg}`}
      >
        {state.replace("_", " ")}
      </span>
      {priority === "skipped" && (
