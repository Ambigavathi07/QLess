...
const [form, setForm] = useState({
    ...
    async function load() {
        try {
const r = await api.get("/hospitals/dashboard");

setList(r.data.hospitals || []);
            ...
            <div className="bg-white border border-[#E2E5E0] rounded-xl overflow-hidden">
                ...
                <table className="w-full text-sm" data-testid="hospitals-table">
                    ...
                    <tbody>
                        ...
                        {list.map((h) => (
                            <tr key={h.hospitalId} className="border-t border-[#E2E5E0] hover:bg-[#EDEDE8]">
                                <td className="p-4 font-medium">{h.hospitalName}</td>
                                <td className="p-4 text-[#5C6661]">{h.address}</td>
                                <td className="p-4 font-mono">{h.doctorsCount}</td>
                                <td className="p-4 font-mono">{h.tokensTodayCount}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
